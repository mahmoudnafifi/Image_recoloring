...
Two new versions:
 1- accept data point and an array of data points, return an array of distances emd2
 2- accept an array of data points, compute all pair distances and return a matrix of distances emd3
...
/******************************************************************************
float emd1(signature_t *Signature1, signature_t *Signature2,
	...
	flow_t *Flow, int *FlowSize)
{
	...
	for (int i=0;i<size;i++)
	{
		...
		w = init(Signature1, Signature2[i]);
		...
		for(XP=_X; XP < _EndX; XP++)
		{
			if (XP == _EnterX)  /* _EnterX IS THE EMPTY SLOT */
				continue;
			if (XP->i == Signature1->n || XP->j == Signature2[i]->n)  /* DUMMY FEATURE */
				continue;
...
float* emd3(signature_t *Signature[], int size,
	flow_t *Flow, int *FlowSize)
{
	float* results = new float[(size*size - size) / 2];
	int count = 0;
	for (int i = 0; i < size; i++)
	{
		for (int j = i + 1; j < size; j++)
		{
			...
			w = init(Signature[i], Signature[j]);
			...
			for (XP = _X; XP < _EndX; XP++)
			{
				if (XP == _EnterX)  /* _EnterX IS THE EMPTY SLOT */
					continue;
				if (XP->i == Signature[i]->n || XP->j == Signature[j]->n)  /* DUMMY FEATURE */
					continue;
			...
			printf("\n*** OPTIMAL SOLUTION (%d ITERATIONS): %f ***\n", itr, totalCost);
#endif
			results[count++] = (float)(totalCost / w);
		}
	}
	return results;
}
